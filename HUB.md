  ...
  permitted:
    ...
    - operation: ask Codex to review a pull request again
      ...
      max: >-
        ...
        allowed, posted no sooner than an hour after the failure. Never to
        re-roll a round that found something, and never while the previous
        round is still running, which the reviewer signals with an eyes
        reaction.
      enforced_by: nothing mechanical.
    ...
    - operation: push a small mechanical change to this repository's default branch
      ...
      shape: >-
        ...
        is what Codex reviews. A port's landed record, which lint requires in the
        same change as the item's done mark, is part of that queue-state change
        and goes with it.
      max: no limit
      enforced_by: >-
      ...
      granted: 2026-09-10 (ruling R15); narrowed 2026-09-25 (Brett).
  permitted_note: >-
...
`ready`, `claim`, `beat`, `release`, `done`, `reconcile`, `fresh`) does the
mechanics; every rule it enforces is written here, once, and nowhere else. Every
number those rules use lives in `queue/config.yaml`, as `constants_live_in`
says. The dated history and the reasoning behind these rules are in
...
`blocked_by`, its `legacy` citation of the bare backlog number, an `evidence`
pointer, a `venue`, and, for a defect, `retires_when`.
...
to each Codex finding there, saying how it was fixed or why it is not a defect,
and leaves resolving the thread to him (Brett, 2026-09-25). For a pull request
in the delegated classes, ruling R16 moved the looking to Codex: mark it ready,
answer what Codex finds, and merge it on the four conditions that section lists.
An agent that cannot tell which list its own pull request is in leaves it a
draft and says so.
...
- **The Codex review has completed**, with every finding either fixed in a push
  ...
  **"Completed" means the last review the agent asked for, not a review of the
  ...
  after such a failure is the re-review row's, no sooner than an hour later. **Code** here means executable
