...
10. For KNB, add the reviewed `metadata/eml-mapping.yml` sidecar, generate
    ...
    representation = "expanded")`. A live
    ...
    The plan contains the original data files plus each validated canonical SDP
    artifact as a named object with its package-relative path. No ZIP is
    required, and the KNB-specific EML mapping and publication receipts remain local. KNB
...
- Generate deterministic, schema-validated EML 2.2.0 from a strictly reviewed
  ...
  `publish_sdp_to_knb()`. Expanded KNB plans retain the original data resources
  and publish the closed canonical SDP inventory as named package-relative
  objects without a ZIP.
