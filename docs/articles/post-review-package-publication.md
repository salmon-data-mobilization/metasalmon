...
3.  re-run semantic suggestions only for fields that are still
    unresolved;
...
7.  rebuild EDH XML if needed;
8.  run strict final validation; and
9.  assemble the **reviewed closure** the EML/KNB path needs before
    publishing.

Step 9 is the one that surprises people. Strict validation is the last
gate the *Salmon Data Package* has; it is not the last gate before a
**deposit**.
...
- **Which required metadata is still a placeholder?** (the warnings
  preview them, deduplicated by file and field — a long list is
  truncated, so treat it as a pointer, not an inventory)
- **Which measurement rows still lack a final semantic term?**

Do **not** switch to `require_iris = TRUE` yet unless you believe the
package is fully finalized. Strict validation is the last gate, not the
first one.
...
That usually produces a much shorter shortlist than the original package
export, because rows you already resolved in Excel are no longer the
target.

### 4) Detect likely ontology gaps

Now detect the places where `metasalmon` still cannot find a shared SMN
...
- if the term would still make sense in a non-DFO salmon dataset, lean
  **SMN**;
...
because the EML/KNB path additionally requires a **reviewed closure**: a
...
`statistical_modifier_iri` and `unit_iri` on your `measurement`
...
Three of its fields are cross-checked against the package and will abort
the build if they drift, so set them from one source rather than
retyping them: `source_provenance.source_citation` and
`source_provenance.provenance_note` must byte-match `dataset.csv`, and
`publication.public` must equal the `public` argument you pass to
[`publish_sdp_to_knb()`](https://salmon-data-mobilization.github.io/metasalmon/reference/publish_sdp_to_knb.md).

A live deposit also requires the sidecar to name **exactly one**
metadata-provider `orcid`, and it must be the ORCID behind the DataONE
token you authenticate with. A dry run accepts any syntactically valid
ORCID, so the placeholder in the shipped template will pass a rehearsal
and be refused by the live call.
...
That dry run targets the **KNB Test Node** (`urn:node:mnTestKNB`), which
is what `knb_environment` defaults to for a dry run. The golden path
develops a package against the test node first and posts to production
once it looks good there, so rehearse in that order:
...
Test objects are never promoted to production: a production deposit is
planned separately, with production credentials, and mints its own
identifiers.
...
coordinating node, resolver, and catalog endpoint together — they never
vary independently — along with which credential is read. Test artifacts
are written under `publication/test/`, so the reviewed production
`metadata/eml.xml` and `publication/knb-manifest.json` are left exactly
as they were.

Live KNB publication is a separate explicit action. It requires a
short-lived DataONE JWT in the process-local option belonging to the
environment (`dataone_token` for production, `dataone_test_token` for
the test node — they are separate credentials), an ORCID-authenticated
subject matching the EML metadata provider, an explicitly named
`knb_environment`, and an explicitly supplied `confirm = TRUE`. A live
call never infers its environment. The confirmation means both that the
exact pending manifest is approved and that the caller has authority to
...
### 12) Publish or share the finished package
...
- keep `term-request-plan.csv` only as working governance support, not
  as part of the canonical final package unless you want that audit
