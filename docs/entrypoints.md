...
- Reviewed suggestion merge: `apply_semantic_suggestions()`; `"reviewed"`
  ...
  repeated constraints in SDP semicolon form; `"top"` remains lexical and
  single-winner
- Harness review (model judgement outside the package, S16):
  `write_semantic_review_packet(path)` -> `review/semantic-review-packet.json`
  -> the harness writes `review/semantic-assessments-pass-1.csv` and its
  `.packet-id` sidecar -> `ingest_semantic_assessments(path)` -> the record in
  `review/semantic-llm-assessments.csv` (read by `semantic_llm_assessments(path)`)
  and the rewritten `semantic_suggestions.csv` -> `review_semantics()` ->
  `accept_suggestion()` -> `apply_sdp_semantics()`. A retry writes
  `review/semantic-review-packet-pass-2.json`; the harness answers it and the
  ingester is called again. `llm_assess = TRUE`, the `llm_*` arguments and
  `chat_decomposition()` are deprecated and removed in 0.7.0
...
- Review packet and assessment ingest (the harness seam) ->
  `R/semantic-review-packet.R`, `R/semantic-review-ingest.R`,
  `R/semantic-review-json.R` (the canonical emitter) and
  `inst/extdata/semantic-review/` (the vendored schema and instructions);
  conformance fixtures in `tests/testthat/fixtures/semantic-review/v1/`,
  regenerated by `scripts/build-semantic-review-fixtures.R`
- Deprecation of the in-package model call -> `R/semantic-review-deprecation.R`
- Context loading, option policy, and the deprecated in-package LLM review ->
