...
| 31 | Idiom (stricter) — **the accept rule converged in S16 step 1 (B-326, 2026-09-25); the column-presence half stands until the removal release** | `apply_semantic_suggestions(strategy = "llm")` requires the frame to carry `llm_selected`, `llm_decision` **and** `llm_confidence` in metasalmonpy, and R requires only `llm_selected`. **Both now apply only rows with `llm_selected` truthy *and* `llm_decision == "accept"`**, and both merges set `llm_selected` only on an accept; R checks the decision when the column is present and still does not demand it. Python's `min_llm_confidence` was accepted and ignored until B-327 | Pre-existing and previously unregistered; amended 2026-09-25 for the review-packet contract. **The R-side justification this row carried named a function that does not exist** — `.ms_llm_validate_assessment_response()`; the real guard is `.ms_validate_llm_assessment()` (`R/llm-semantic-helpers.R`), which clears the index on a non-accept decision, and `llm_selected` (`R/semantic-suggestions.R`) was derived from that index alone. Each language had one of the two guards: R cleared the index, Python required `accept` in the merge and in apply. **That was enough while the package's own model call wrote the row, and not once a harness writes it**: the index and the decision are two separate cells of a file, so the ingester's merge (`.ms_semantic_merge_llm_assessments()`) now requires `llm_decision == "accept"` as well as the matching index, and `apply_semantic_suggestions(strategy = "llm")` filters on the decision whenever the frame carries it, in both languages (execplan §3.5 and §8). What remains of the row is column presence: R applies a frame that carries `llm_selected` alone, Python refuses one without all three. **Retirement condition:** the removal release (0.7.0, decision 15 of the S16 execplan) makes R require the three columns too, and metasalmonpy honours `min_llm_confidence`; the row then records the convergence |
...
| 62 | Legacy (the deprecated in-package model call; **retires with the removal release**) | Every difference between the two in-package model-call paths that no row registers, gathered under one number because they share one retirement: **chunking and scoring** (R cuts 2200-character chunks with 200 of overlap, scores by token set and keeps 4 excerpts; Python cuts 1400 with no overlap, scores by substring, keeps 8 and collapses whitespace), **validator semantics** (row 65), **exploration** (R asks the model for alternate queries on low-confidence answers of several kinds; Python retries on `retry_search` only), **the escalation precondition** (R escalated a rejection only when the pre-retry decision was also a rejection until B-361 moved it to any final rejection, which was Python's rule), **zero-candidate targets** (Python assesses them, R did not), **shortlist depth**, **request bodies** and **the environment variables read** (R nine, Python a `{PROVIDER}_MODEL` and `{PROVIDER}_BASE_URL` per provider plus four keys, and a default OpenAI model where R aborts) | Measured 2026-09-25 for the S16 execplan (§0 and §1) at metasalmon `ae16b42` and metasalmonpy `f1f7230`. Registered as one *legacy* row rather than one row per difference (decision 16 of the execplan): the path these differences live in is deprecated in both packages as of S16 step 1 and is deleted by S16 step 3 (B-329 / B-330), so nothing here is worth converging, and the one thing worth recording is that the differences exist and are known. **None of them reaches the review-packet contract**, whose context algorithm, escalation rule, zero-candidate handling and depth are one design in both languages (rows 63 and 64 hold the two that do differ there). **Retirement condition:** the removal release deletes both paths; this row retires by deletion with the code, in the pull requests that delete it |
| 63 | Idiom | Three things in the review-packet contract are implementation-specific by design: **(a)** the packet's `producer` member (`implementation`, `version`, `function`), which names the package that wrote it and is the one member excluded from `packet_id` and from the cross-language byte comparison; **(b)** how the deprecation of the in-package model call is detected — R with `missing()` over the eleven `llm_*` arguments, so an explicitly supplied default counts, Python with any non-default value, because it cannot tell an explicit default from an omitted one — and how it is classed, a `cli_warn` of class `metasalmon_llm_deprecated`/`deprecatedWarning` silenced by `options(metasalmon.llm_deprecation_quiet = TRUE)` against a `FutureWarning` subclass silenced by a warnings filter; **(c)** schema enforcement: neither validates a packet against the vendored JSON Schema at runtime — the schema is the contract's specification, checked in tests (R skips without `jsonvalidate`) — and an error row's `llm_error` text is worded by each implementation (the fixtures compare its presence, not its words) | Each is the smallest honest difference: a packet has to say who wrote it, a language can only detect what its calling convention lets it see, and a runtime schema check would make the packet's own emitter and its validator two authorities on one shape. Registered 2026-09-25 with S16 step 1 (B-326 / B-327) per §8 of the execplan. **Retirement condition:** (a) never — a packet built by either package must say so; (b) retires with the removal release, when there is no deprecated call left to detect; (c) retires if the ecosystem adopts one runtime validator for both packages, which nothing plans |
| 64 | Idiom (permanent) | Context text extracted from **PDF, DOCX, XLSX and HTML** may differ by library between the two packages (`pdftools`, `xml2` and `readxl` in R; the Python extractors on the other side), so a packet whose context inputs include those formats is not byte-identical across languages even on identical inputs. Text formats (`md`, `txt`, `csv`, `tsv`, `json`, `yaml`, `rst`, `r`, `rmd`, `qmd`) are extracted identically: UTF-8, then cp1252, then latin-1; CRLF to LF; front matter and fences dropped; no whitespace collapsing | The cross-language bar the execplan sets (§2.7, decision 4) is byte identity except `producer` **on shared fixtures with injected candidates and text-only context**, which is exactly what the conformance fixtures hold, and this row is the part of the bar that is not reachable: two libraries do not agree on where a PDF's line breaks fall. The packet records each context input's SHA-256, so a mismatch is visible as an input difference rather than a contract failure. Registered 2026-09-25 with S16 step 1 (B-326 / B-327). **Retirement condition:** never; recorded so that a byte differential over a packet with binary context inputs is not read as a defect |
| 65 | Gap (open — **closes with B-360 in the same release train**) | The bundle validators the ruling keeps are not at parity: R's dimension classifier covers area, volume, flow and speed with compound rules and Python's covers none of that; the field anchors differ; R's method-evidence pattern has two alternatives Python lacks; and the two split role hints differently. The shared ingest fixtures run the validators at every ingest, so they fail on the Python side until it converges | Found by the S16 execplan (§0.7) reading both validator files on 2026-09-25, and ruled the same day (decision 9): converge on R's validators, with Python moving, because they are the richer and more recently hardened set — a direction decided for this divergence, not an "R leads" default. The convergence is hub item **B-360**, sequenced before B-327 so that the fixtures pass there. **Retirement condition:** B-360 lands and metasalmonpy's copy of `tests/testthat/fixtures/semantic-review/v1/` passes, including `bundle_validator_downgrade` and the two adversarial Theme A cases; this row then records the convergence and its date |
...
nothing-to-migrate early return now builds the same three-column
`report$tables` frame — `table_id`, `method_iri`, `columns` — that its
populated build and its no-placement return already built, so a caller reading
`report$tables$columns` gets an empty character column instead of `NULL` in
exactly the case where the package was already clean (backlog #112, pull
request #117). Brett ruled the three-column shape on 2026-09-14, for both
implementations. The Python counterpart is `migrate_sdp_methods()` in
`sdp_methods.py`, whose no-op exit returns
`pd.DataFrame(columns=["table_id", "method_iri"], dtype=object)` at
`sdp_methods.py:1086-1088` while its placements frame at `:1141` already names
all three. Queued as **B-144**, blocked by B-112. It did not land in the same
stream because the hub claim that produced #117 covers one branch in the item's
repository, so the port is a separate queue item rather than a deviation row.

**Two things make this one unlike the two ports above, and both are easy to get
backwards.** **(1) It runs backwards.** metasalmonpy had the three-column frame
*first* and gave it up at S10 chunk A to match R, so B-144 restores what this
package originally had rather than adding something it never did. It is still a
port and not a deviation — the two are meant to agree and currently do not — but
it is the amended mirror contract's own case (Brett, 2026-08-17: the presumption
of mirroring is about keeping the two the same, never about which one is
correct), so nothing here should be read as Python lagging. **(2) The Python
code documents the defect as intended**, which makes B-144 a comment correction
as well as a code change. The lines above the frame at `sdp_methods.py:1083-1085`
read *"Two columns, not three: R's nothing-to-migrate report frame has no
``columns`` column (unlike the empty placements frame the stop-free path
returns), and the differential run showed it."* That is accurate about what the
differential saw and wrong about what the shape should be, and a port that fixes
the frame while leaving the comment standing would leave the next reader an
...
now renders as readr's ISO instant form — the `T` separator and the `Z` zone
marker — so `datapackage.json` and `metadata/dataset.csv` no longer spell one
instant two ways (backlog #115, hub item **B-115**). Brett ruled the spelling on
2026-09-14 **for both implementations at once**, precisely so that neither side's
implementer picks one; the Python counterpart is `package_io._clean()` and the
metadata frame's `to_csv` path, and it has the same defect mirrored (`T` form in
the descriptor, space form in the CSV). Queued as **B-145**. It did not land in
the same stream because the hub claim that produced the R half covers one branch
in the item's repository.
**This one is a port and a deviation row at once, which is unusual enough to say
plainly.** The port is B-145 and is owed; **no new numbered row was added for
it**, because **row 56** already owns this divergence and named this very
ruling as its retirement condition. Row 56 was therefore updated **in place** —
recording the ruling, that R moved, that B-145 owes the same move, and the
year-padding residual B-145 has to measure — following the chunk-F precedent of
rewriting a row and claiming no number. Read row 56 as the record of a *known,
one-sided, temporary* divergence awaiting B-145, **not** as a statement that the
difference is wanted. Claiming a number here would have been worse than
redundant: `scripts/check-parity-registers.py` fails on a number present in one
...
**Row 59 was added 2026-08-25 and its twin has not landed yet — the mirror
image of the rows 54–58 situation above.** The pass that added it had write
access to this repository and not to metasalmonpy (a deliberate constraint of
the Q12 implementation stream, which was told not to edit the mirror), so it
spent the number here and handed the `PARITY.md` twin text over in its report.
The checker reads numbers, so **it will report 59 as one-sided until a
metasalmonpy change applies the twin**, exactly as it reported 54–58. That
transient is expected and is not drift; what would be drift is the number being
spent on one side and the *content* never following. Whoever applies the twin
should delete this paragraph in the same change, and can measure the row rather
than trust it: build one in-memory mapping set with a typed `confidence` column
in each implementation and diff the canonical bytes.
