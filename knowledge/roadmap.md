...
- **The model call leaves the packages in two releases, additive first**
  ...
  The harness skill that replaces the in-package call is released between the
  two, in the plugin, against a released package. Detail and the items:
...
**The gap is closed**: `v0.5.0` (2026-08-25) is the newest tag. `v0.4.0`
(2026-08-24) closed the 153-commit post-0.3.0 gap per Q7, and `v0.5.0` released
S5's 35 commits one day later — so `main` and the newest tag are the same
package again, and have been at every point since 0.4.0. Details under
...
> index and both repositories' `AGENTS.md` — were at 0.4.0 with no catch-up
> window open.
>
...
### salmon-data-standards-workshop — commit-cited curriculum

| Version | Date | One line |
|---|---|---|
| No local release tag; cite commit | 2026-09-08 | Published two-day curriculum [`27e0ced`](https://github.com/salmon-data-mobilization/salmon-data-standards-workshop/tree/27e0cede5709de4bfebb9894c45a038259305456): 12 chapters, 720 minutes; local checks and deployment passed; public chapters and ZIP verified. |
| No local release tag; cite commit | 2026-09-08 | Published Day 1 baseline [`190df307b4e486d97f9899fa26c4bdff376a66dd`](https://github.com/salmon-data-mobilization/salmon-data-standards-workshop/tree/190df307b4e486d97f9899fa26c4bdff376a66dd): seven human-first chapters, 360 minutes, one unchanged 173×14 Fraser Coho 2023–2024 source; metasalmon 0.5.0 / metasalmonpy 0.4.0. |

The workshop is the eighth ecosystem member under Brett's 2026-08-24 ruling;
[S4](sequences/s4-workshop-rebuild.md) owns its current curriculum state.
The authorized Day 2 expansion adds five chapters (60/75/90/75/60 minutes)
for another 360 minutes of semantic practice and contribution stewardship.
Implemented and pushed directly to `main` at [`27e0ced`](https://github.com/salmon-data-mobilization/salmon-data-standards-workshop/tree/27e0cede5709de4bfebb9894c45a038259305456)
on 2026-09-08. Local rendering, links/downloads, vocabulary/model/bridge checks
and both pinned request-preview lanes passed. The [deployment workflow](https://github.com/salmon-data-mobilization/salmon-data-standards-workshop/actions/runs/34304171873)
passed; the public chapters and ZIP were verified against the committed
content on 2026-09-08 (Pacific time). The [2026-09-08 evidence card](workshop-curriculum-and-sdo-guidance-2026-09-08.md)
records the baseline, API preview checks, and draft SDO documentation findings.

The install/lockfile split and missing primary-key explanation described in
the August history are superseded by the present lesson. Python's missing
native R review/setter capabilities remain stated. The technical reference
still awaits Bruno and Tom's scientific review; the independent-validator
mismatch, live free-provider rehearsal, and verified public KNB record remain
unresolved as recorded in the kit. A deployed teaching site is not a successful
...
> **RULED 2026-08-22 (Brett), option A.** Kept in place with its heading
> unchanged, as OD-1 is, so that every existing link to this anchor still
> resolves. The ruling goes here, below the heading, and not into it: appending
> it to the heading changed the generated anchor and broke the four inbound
> links in this card and in the S3 and S4 sequence cards, which is exactly what
> the convention above exists to prevent.
...
**Shipped: 0.5.0** (tagged `v0.5.0`, GitHub Release published 2026-08-25).
...
| 0.5.0 | S5 R-native review and editing — #74, #118, #60's accessor clause; nine new exports and the two accessors, and the spreadsheet leaves the documented path |
...
**0.5.0 releases S5**, cut 2026-08-25 — 35 commits on top of `v0.4.0`. It is a
**minor** for the reason 0.4.0 was not quite: nine new exported functions and a
materially new workflow, with nothing renamed, removed, or changed in
...
**Workshop-specific update, 2026-09-08:** the authorized curriculum now uses
local/reference artifacts when live services are unavailable. The historical
S3→S4 dependency below still describes the live rehearsal, not a prohibition
on authoring or teaching the local curriculum. General OD-2 and other stream
...
baseline was **0.4.0, released**, and is now **0.5.0, released** (2026-08-25).
**That requirement was satisfied at 0.4.0 and is one release stale again:** the
...
downstream, and the 0.5.0 shortfall is **S5's port to carry, not S10's** — the
stream stays closed rather than reopening every time R releases. The first
behavior is verification,
...
- [S9 — Ontology conventions and alignment pass](sequences/s9-ontology-alignment.md) · step 7's four decisions are made; gcdfo #67 is closed, #68–#75 stay open behind successors #84/#85 and smn PR #27. **2026-09-02 source pass** (four commons cards; the citation ledger it was said to have produced was never written, measured 2026-09-10): Ogden 2015 §2.3 located — #85 half-unblocked, but the SPSR integers run the opposite way to the report (new decision 9); "management level" has no published DFO counterpart (#84 precedent favours reading (a)); PR #27 needs annotation amendments the pass reached in prose but never emitted as a file, so that input has to be rebuilt from the commons cards before the eight rulings (queue `B-121`) — two attributions fail the passage check
...
repository. Every queue item it files is in a member repository, and its one
step outside them is in the plugin, which is not a member. Whether naming that
step here amounts to this hub sequencing the plugin's work, which is the
membership test [Q10](questions.md) adopted, is Brett's question, and nothing
here assumes the answer.
