...
- [S3 — KNB staging environment](sequences/s3-knb-staging.md) · **R side implemented 2026-08-22, released in metasalmon 0.4.0 and mirrored in metasalmonpy 0.4.0 (both 2026-08-24)** — `knb_environment` with a closed two-environment registry, dry runs defaulting to the verified KNB Test Node; the Python mirror was one of the two gaps the 0.4.0 parity audit found absent, because the R original landed after every S10 chunk was written. **Rehearsal half closed 2026-08-25:** the first end-to-end test-node deposit succeeded — ten objects to `urn:node:mnTestKNB`, `status: published_pending_catalog`, from the shipped Fraser coho example, with `scripts/build-fraser-coho-knb-rehearsal.R` as the reproducible recipe. That was the last thing S4's golden-path section waited on. **Still outstanding:** no *production* deposit, and the reviewed closure the deposit needed has no exported producer (#116)
- [S4 — Workshop rebuild](sequences/s4-workshop-rebuild.md) · **partially landed 2026-08-25** — version pinning (both lanes to `v0.4.0`), tidy-shape/`primary_key` content, the KNB golden path and `migrate_sdp_methods()` shipped as workshop PR #5. **Brett is rewriting sessions 2/3/4/6 directly on `main` in parallel**, and his `24b9da3` rewrote the semantic-review episode that **S5 replaces** — that collision is unruled. The card's pre-2026-08-25 inventory was substantially stale and has been corrected against measurement
