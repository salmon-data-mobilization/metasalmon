...
Declare a `primary_key` naming the column(s) that uniquely identify a row —
here each row is one (population, year). Since 0.2.6 a declared key is
**enforced**: duplicate or missing key values are hard validation issues, which
is exactly the tidy-data guarantee you want a reviewer to rely on. Include
extra columns such as `spatial_extent` or `temporal_start` when they help
others understand the scope.

One contract worth knowing when you author CSVs by hand: since 0.2.4 the
canonical missing-value token is the **empty field**. Cells containing the two
characters `NA` are read as the literal string `"NA"` — rewrite them as empty
fields.
...
writes the same XML automatically to `metadata/metadata-edh-hnap.xml`. That
create-time XML is a **draft** whenever the review-ready metadata still contains
`REVIEW:` IRIs or `MISSING` placeholders; `metasalmon` writes it but warns so it
is not mistaken for a finalized EDH submission.
...
edited in Excel before EDH submission. It refuses to run until obvious review
...
explicitly creates a strict allowlist for both initial and retry retrieval.
