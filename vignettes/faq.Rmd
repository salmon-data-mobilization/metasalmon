...
missing-value token in every SDP CSV. Cells containing the two characters
`NA` are read as the literal string `"NA"`, which matters when a code list
legitimately uses `NA` as a code (for example "not assessed"). If you have a
hand-authored package that used `NA` to mean missing, rewrite those cells as
empty fields. Inside R, an `NA_character_` value is written out as an empty
field automatically, so packages written by metasalmon already follow the
contract.
...
For LLM context, plain-text and CSV files with invalid UTF-8 are retried as
Windows-1252/Latin-1. R Markdown and HTML context files are still expected to be
UTF-8.
...
- `llm_context_files` must contain local path strings such as
  ...
  `readr::read_csv()` or the XML object from `xml2::read_html()`.
- Context never enables LLM review by itself. Set `llm_assess = TRUE` and choose
  a configured provider. Without that flag, `metasalmon` warns and continues
  with deterministic semantic retrieval, so similar output is expected.

When using `infer_dictionary()` directly, also set `seed_semantics = TRUE`.
Use `llm_context_text` instead when the context is already a character string
rather than a file.

### Why did semantic review ignore or omit a vocabulary?

Source selection distinguishes an omitted argument from an explicit value:

- omit `sources` in `suggest_semantics()` or `semantic_sources` in
  `create_sdp()` to use role-aware defaults;
- supply a vector to make it a strict allowlist for initial and retry
  retrieval.

For example, `sources = "smn"` deliberately excludes QUDT even for a unit slot.
Use `sources_for_role("unit")` when you want the normal unit vocabulary set.

For measurement columns, the LLM reviews variable, property, entity, unit,
constraint, and statistical-modifier candidates together. Only variable,
property, entity, and unit acceptances can become `REVIEW:` prefills.
Constraint and statistical-modifier decisions always require manual
application, and deterministic validators can downgrade unsupported model
acceptances to `review`.

A method is not among them: since sdp-0.3.0 a method is never a dictionary
column. See
[Migrating to SDP 0.3.0](migrating-to-sdp-0-3-0.html) for where it lives now.
...
- [Report a bug](https://github.com/salmon-data-mobilization/metasalmon/issues/new?labels=bug)
- [Request a feature](https://github.com/salmon-data-mobilization/metasalmon/issues/new?labels=enhancement)
