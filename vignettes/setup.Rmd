---
title: "Setup and Credentials"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Setup and Credentials}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

```{r, include = FALSE, purl = FALSE}
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>",
  eval = FALSE
)
```

Use this page once before the homepage quick example or the [5-Minute Quickstart](https://salmon-data-mobilization.github.io/metasalmon/articles/metasalmon.html).

It covers three things:

1. GitHub credentials so `remotes::install_github()` works reliably.
2. Optional LLM provider setup if you want `llm_assess = TRUE` later.
3. Short-lived DataONE credentials for an explicit live KNB deposit.

## 1) GitHub setup for installs and updates

`metasalmon` is installed from GitHub. Public installs often work without a token, but setting up a GitHub Personal Access Token (PAT) up front avoids rate limits and awkward auth failures.

### Create a GitHub token

Create a PAT in GitHub:

- <https://github.com/settings/tokens>

For public package install/update, a read-only token is enough. If you also plan to read private GitHub repositories later, make sure the token can access those repositories too.

### Store it in `~/.Renviron`

```{r, purl = FALSE}
file.edit("~/.Renviron")
```

Add:

```{r, purl = FALSE}
GITHUB_PAT="paste token here"
```

Restart R after saving.

### Install `metasalmon`

```{r, purl = FALSE}
install.packages("remotes")
remotes::install_github("salmon-data-mobilization/metasalmon")
```

### If you also need private GitHub CSV access later

The install token and the package's private-repo CSV helpers are related but not identical workflows.

When you later want to use `read_github_csv()` against a private repository, run:

```{r, purl = FALSE}
ms_setup_github(repo = "your-org/your-private-repo")
```

That verifies private GitHub access for the repository you actually intend to read.

## 2) Optional LLM provider setup

The base quick example and quickstart do **not** require an API key.

Only do this setup now if you want to run:

- `create_sdp(..., llm_assess = TRUE)`, or
- `suggest_semantics(..., llm_assess = TRUE)`

If you also plan to pass PDF or Excel files through `llm_context_files`, install the optional readers once. HTML, DOCX, `.R`, `.Rmd`, and `.qmd` inputs work without extra reader packages:

```{r, purl = FALSE}
install.packages(c("pdftools", "readxl"))
```

### DFO internal: `chapi`

If you are on the DFO internal network or VPN, open:

- <https://chapi-dev.intra.azure.cloud.dfo-mpo.gc.ca/>

Then:

1. click the user icon in the bottom left,
2. open **Settings**,
3. click **Show** next to **API Keys**,
4. copy the key value.

Store it in `~/.Renviron`:

```{r, purl = FALSE}
file.edit("~/.Renviron")
CHAPI_API_KEY="paste key here"
```

Optional overrides:

```{r, purl = FALSE}
CHAPI_MODEL="ollama2.mistral:7b"
CHAPI_BASE_URL="https://chapi-dev.intra.azure.cloud.dfo-mpo.gc.ca/api"
```

### External users: OpenRouter

```{r, purl = FALSE}
file.edit("~/.Renviron")
OPENROUTER_API_KEY="paste key here"
```

`llm_provider = "openrouter"` defaults to `openrouter/free`.

### External users: OpenAI

```{r, purl = FALSE}
file.edit("~/.Renviron")
OPENAI_API_KEY="paste key here"
```

Then choose an explicit OpenAI model when you call the LLM review path, for example:

```{r, purl = FALSE}
suggested <- suggest_semantics(
  df = your_data,
  dict = your_dict,
  llm_assess = TRUE,
  llm_provider = "openai",
  llm_model = "gpt-4.1-mini"
)
```

## 3) Optional DataONE/KNB live-deposit credential

You do not need a DataONE credential to build EML or create a KNB dry-run
manifest. For a live deposit, sign in to DataONE with the same ORCID recorded as
the single EML metadata provider and obtain a short-lived DataONE JWT.

Treat this token differently from an install PAT or optional LLM key. Do not put
it in `.Renviron`, a script, a command argument, an EML sidecar, a publication
manifest, or a transformation record. Enter it only into the current R process
and clear it immediately afterward:

```{r, purl = FALSE}
options(dataone_token = rstudioapi::askForPassword("Short-lived DataONE JWT"))
on.exit(options(dataone_token = NULL), add = TRUE)
```

The live KNB preflight verifies that DataONE reports the authenticated subject
as the metadata-provider ORCID. A matching string supplied locally is not
enough.

## 4) Next step

After this setup is done, go back to:

- [Homepage quick example](https://salmon-data-mobilization.github.io/metasalmon/)
- [5-Minute Quickstart](https://salmon-data-mobilization.github.io/metasalmon/articles/metasalmon.html)
- [LLM Review With Context Files](https://salmon-data-mobilization.github.io/metasalmon/articles/llm-context-review.html)

If you are continuing from a reviewed package later, use:

- [After Excel Review](https://salmon-data-mobilization.github.io/metasalmon/articles/post-review-package-publication.html)
